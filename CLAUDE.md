...
The database accumulates rows for files that have since been deleted or moved. Directory-level reconciliation is built in — `--prune-dirs` (added 2026-07-16; see Pruning stale database rows) deletes the rows of directories that no longer exist, pulled in as the same kind of cheap increment over existing machinery that justified `--verify`. Finer-grained pruning (per-file staleness, moved-file detection) remains future, higher-level tooling, consistent with sumtag's single-purpose scope.
...
## Pruning stale database rows (`--prune-dirs`)
...
- **Directory-level, deliberately.** A deleted directory takes exactly its *resident* file rows with it (dirname equality, never a recursive prefix delete). Recursion is unnecessary by construction: a vanished parent implies vanished children, and every child directory that held files is independently in the check list, so it is discovered by its own check. The trade accepted (decided 2026-07-16): a file deleted from a directory that still exists is not noticed, and a directory moved elsewhere is pruned at its old location like any other deletion — the intended use is whole-directory deletions, where this is exactly right.
...
**Conflicts:** `--prune-dirs` requires `--database` (it acts only on the database) and requires explicit scan roots like every mode. It conflicts with `--sum`/`--import`/`--locate`/`--verify`/`--remove` (one run, one mode), with `--force` (no re-hash decision to override), and with `--prescan`/`--db-prescan` (nothing is checksummed; its own counter is built in). `--exclude` and `--no-ignore` are errors with it, not silent no-ops: they govern filesystem traversal, and `--prune-dirs` walks the database, not the filesystem. `--prune-dirs -n` is allowed and is the intended preview.
...
`--prep` = stages 1+2; a bare `grouper.py --database DB` prints the stored grouping. Inspection helpers: `--ls DIR`, `--compare A B`, `--dups`, `--top [N]`. Comparison functions are registered by name in `COMPARISONS` as pure **(signature, scorer) pairs** (reshaped 2026-07-15): `signature(files)` distills one directory's file-list into a plain picklable value, built **once per directory** — profiling showed rebuilding it per pair, N−1 times each, dominated the whole `--pairs` phase (~16× once hoisted) — and `score(sig_a, sig_b)` does the pairwise math. The functions: `digest` — content only, renames free; `name-digest` — all-or-nothing per (name, digest); `name-score` — name-anchored partial credit: 1 point for a shared basename, +2 if the digests also agree.
...
- **Safety checks**, all before anything is at risk: roots must exist and be **disjoint under `realpath`** (not equal, neither nested — symlinks can't disguise overlap); the database must already exist and know both roots (live mountpoint recorded, rows present — an unmounted drive errors instead of matching nothing); mixed digest algorithms across the roots are refused without `--allow-mixed` (the documented mixed-algorithm hazard: cross-algorithm duplicates silently survive). At the moment of deletion, `realpath(cull file) == realpath(witness)` means one directory entry reached through links — refused loudly (exit 2), since deleting "the copy" would delete the only name; same inode with *different* realpaths is a genuine hard link — safe, deleted, noted.
