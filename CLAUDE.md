...
The database accumulates rows for files that have since been deleted or moved — most acutely when whole directories are removed. `--prune-dirs` (added 2026-07-16; move/rename detection added 2026-08-05) reconciles it: for each scan root it collects **every directory the database knows** under that root (the distinct dirnames of its file rows — the schema stores no directory table, so directories are derived from the rows themselves), checks each with one `stat`, and deletes the rows of directories that no longer exist. It was pulled into sumtag as the same kind of cheap increment over existing machinery that justified `--verify` — a step toward richer verify-style auditing — while per-file staleness and moved-file detection stay out (see Future work).

**`--prune-all`** (added 2026-07-16, same day, closing `--prune-dirs`'s documented trade) does everything `--prune-dirs` does **and** checks per-file staleness: after a directory is confirmed to still exist, each of its resident file rows gets one `lstat`, and rows whose file no longer exists — or is no longer a *regular* file (a path that turned into a directory or symlink is not the stamped file) — are deleted. The directory pass still runs first, so a vanished directory costs one `stat` total, never one per resident row; the extra cost of `--prune-all` over `--prune-dirs` is one `lstat` per row **in surviving directories only**, which is why the two stay separate flags: on an archive where whole-directory deletion is the norm, `--prune-dirs` is much cheaper and usually enough. Giving both flags together is redundant but not an error (`--prune-all` subsumes `--prune-dirs` — the `--locate`-implies-`--import` idiom). Everything below applies to both flags equally; per-file prune announcements follow the same bare-path/`-v` rule (`prune <path> (file row)` with `-v`), the summary's `checked:` line adds a file count, and `--progress`'s counter unit becomes `paths` (directories + file rows). What `--prune-all` still does **not** do: notice an *individually* moved file (pruned at its old location like any deletion — only whole directories are matched to new locations), or anything content-based — no hashing, ever; one `lstat` is the entire per-file check.

- **Directory-level, deliberately** (`--prune-dirs`). A deleted directory takes exactly its *resident* file rows with it (dirname equality, never a recursive prefix delete). Recursion is unnecessary by construction: a vanished parent implies vanished children, and every child directory that held files is independently in the check list, so it is discovered by its own check. The trade accepted (decided 2026-07-16): a file deleted from a directory that still exists is not noticed (that is `--prune-all`'s job). A directory moved or renamed is no longer pruned as a deletion — see the move-detection bullet below (added 2026-08-05).
- **Move/rename detection (added 2026-08-05; design recorded in TODO.md, all questions resolved).** Before a vanished directory's rows are deleted, `--prune-dirs` checks whether the directory merely moved or was renamed — using the file rows' recorded **inodes**, with no schema change: lost directory L matches candidate F iff **every** resident row of L has a regular-file child of F agreeing on all three of **(inode, basename, file_mtime to the stored microsecond)** — the dedupe trust-veto idiom, zero content I/O (inode numbers recycle, so inode alone is never trusted; the microsecond-mtime agreement makes reuse collisions essentially impossible, since `rename(2)` preserves children's mtimes). A match rewrites the resident rows' `rel_path` in place (`move <old> -> <new> (N file rows)`; `would move` under `-n`), preserving row identity, digests, timestamps, and locate columns; `mountpoint_id` never changes (a cross-filesystem move is copy+delete with new inodes and can never match — candidates on a different device than the mount are rejected outright). Candidates are directories holding **zero** database rows, found in two tiers: each lost directory's own *parent* first (one `readdir` — renames-in-place, the common case, resolve for nearly nothing), then, only if unmatched lost directories remain, an **announced** tree-wide walk of the roots (`N vanished directories unmatched; walking roots to check for moves` — approximation-by-consent); after the walk the whole pool is re-resolved, stricter never looser. The walk uses the shared walker, so `@sumtag-ignore` and `--exclude` govern the candidate search — which is why those flags now compose with the prune family. The subset direction tolerates files *added* after the move; anything deleted/renamed/modified since breaks the match and falls back to prune-plus-rescan, the safe direction. A moved *tree* resolves directory by directory (parent and children are independently lost, found, and matched — the same non-recursive composition as the prune itself). **Ambiguity** — one lost directory matching several candidates, or several claiming one (possible only via hard-link farms, where links share inode and mtime) — is refused, never guessed: a loud warning, rows kept, exit `2` (the check was knowingly incomplete). Moves count as stale-found for exit `1`, the summary gains a `moved:` line, and commits stay per directory.
...
- **Type indicators on action lines** (added 2026-07-22): each `delete`/`sweep`/`rmdir` announcement ends its path with an `ls -F` style one-character type indicator (`_ftype_indicator`, one `lstat` from `Run.say`): `@` symlink, `/` directory, `*` executable file, `|` FIFO, `=` socket; a plain file gets none, and the symlink test comes first so a link reads `@` regardless of target. Every live `say()` fires *before* the removal, so the path still exists to stat. Offline (`-n`) omits the indicator — reading a type is filesystem access, which offline forbids, and offline rows are plain files regardless (symlinks and directories are never stamped, so never mirrored).
...
- **Tolerates a live tree changing under the run** (hardened 2026-07-22 after a crash — see BUGS.md): the walk wraps every filesystem touch, so a directory or file deleted concurrently (e.g. the operator manually deleting from the cull tree during a run) is skipped with a warning, never a traceback. A vanished cull directory (`FileNotFoundError` from `_scan`) is treated as already-gone and no longer blocks its parent; a vanished actual directory just witnesses nothing (cull files kept — safe); a file gone between `scandir` and `stat` is skipped; and the armed `os.remove`/`os.rmdir` swallow `FileNotFoundError` (already gone) while any other `OSError` is reported and keeps the directory a blocker rather than crashing. Per-directory commits mean a crash-or-interrupt leaves the completed prunes persisted and the run safely re-runnable.
...
| `--prune-dirs` | | bool | Check every directory the database knows under the given roots; delete the rows of directories that no longer exist, first checking whether each vanished directory merely **moved or was renamed** — matched by its files' recorded (inode, basename, microsecond-mtime) — and rewriting its rows in place if so (see Pruning stale database rows). Requires `--database`; the filesystem is never modified. Exit `0`/`1`/`2` = nothing stale/pruned-or-moved/errors (ambiguous moves are exit 2). Composes with `-n` to preview, `--progress` for a live nnn/mmm counter, and `--exclude`/`--no-ignore` (they govern the candidate walk). |
