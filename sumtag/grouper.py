...
Usage (pipeline order):
    grouper --database DB --prep [--fn N] [--jobs N] [--progress]
                                           ...
                                           #   store the pairs
    grouper --database DB --index [--progress] [--no-junk-filter]
                                           # stage 1 alone
    ...
    grouper --database DB --threshold 0.7  # stage 3: build + persist groups
                                           ...
                                           #   report them
    grouper --database DB [--sort K]       # THE report: show stored grouping
    ...
    grouper --database DB --dupes [--min N]    # duplicate *files* report
...
def _set_meta(conn: sqlite3.Connection, artifact: str, fn: str,
    ...
    from before the column existed); name_match records the --name gate
    (NULL = off). Older databases predate these columns, so they are added
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.clean_db and (args.prep or args.index or args.pairs
                          ...
                          or args.compare is not None or args.dupes
